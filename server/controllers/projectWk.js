...
exports.getProjects = async (req, res) => {
  try {
    const [projects] = await db.execute(`
      ...
      FROM projectsWk p
      LEFT JOIN project_techWk pt ON p.id = pt.project_id
      LEFT JOIN tech_stacksWk t ON pt.tech_id = t.id
      GROUP BY p.id
    `);
...
exports.putProjects = async (req, res) => {
  try {
    ...
    const [existing] = await db.query(`SELECT * FROM projectsWk WHERE id = ?`, [
      id,
    ]);
    ...
    await db.query(`DELETE FROM project_techWk WHERE project_id = ?`, [id]);
...
exports.deleteProjects = async (req, res) => {
  ...
  try {
    ...
    const query = "DELETE FROM projectsWk WHERE id = ?";
